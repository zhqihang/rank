...
def upload(request):
    if request.method == 'POST':
        ...
        index = datas.columns[0]
        datas = datas.drop_duplicates(index, keep='first')
        datas.set_index(index, inplace=True)
        df_range_introduction = pd.DataFrame(df, columns=[index, datas.columns[6], datas.columns[7]])
        df_range_introduction.set_index(index, inplace=True)
        range_introduction_iterrow(df_range_introduction, button_type)
        df_range_introduction.rename(columns={datas.columns[6]: 'profession_name', datas.columns[7]: 'pos_neg'}, inplace=True)
        df_new = pd.concat([datas, df_range_introduction], axis=1)
        df_new.to_excel(excel_writer=filename)
        print(df_new)
        ...
        for i in contact_list:
            ...
            row_list.append(i.profession_name)
            row_list.append(i.pos_neg)
            doc_list.append(row_list)
        ...
        if os.path.exists('/root/industry_software/industry_software/dataset/upload_score.xlsx'):
            df_score = pd.read_excel('/root/industry_software/industry_software/dataset/upload_score.xlsx')
            # 将数据添加到数据库
            for index, row in df_score.iterrows():
                # Rank表的部分字段移动到Company表，这里要进行改进
                rank = models.Rank()
                rank.name = row[0]
                rank.assets = row[1]
                rank.income = row[2]
                ...
                rank.profession_name = row[9]
                rank.pos_neg = row[10]
                rank.score = row[11]
                rank.type = button_type
                rank.save()

            # os.remove('/data/zhanghongbin/industry_software/industry_software/dataset/upload_score.xlsx')
            return render(request, 'upload.html', {'alarm': '上传成功'})

        return render(request, 'upload.html', {'alarm': '上传失败, 数据已存在'})
